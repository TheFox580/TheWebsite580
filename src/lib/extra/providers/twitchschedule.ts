...
import {
  AUTH_TWITCH_SCHEDULE_ID,
  AUTH_TWITCH_SCHEDULE_SECRET,
} from "$env/static/private";

export interface TwitchScheduleProfile extends Record<string, any> {
  sub: string;
...
export default function TwitchSchedule(
  config: OIDCUserConfig<TwitchScheduleProfile>,
): OIDCConfig<TwitchScheduleProfile> {
  return {
    issuer: "https://id.twitch.tv/oauth2",
    id: "twitch-schedule",
    name: "Twitch Schedule Editor",
    type: "oidc",
    client: { token_endpoint_auth_method: "client_secret_post" },
    clientId: AUTH_TWITCH_SCHEDULE_ID,
    clientSecret: AUTH_TWITCH_SCHEDULE_SECRET,
    authorization: {
      params: {
        scope: "openid user:read:email channel:bot channel:manage:schedule",
      },
      claims: {
        id_token: {
          email: null,
